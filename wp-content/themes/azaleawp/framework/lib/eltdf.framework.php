...
class AzaleaEltdfFramework
{
    ...
    private function __construct() {
        $this->eltdOptions = AzaleaEltdfOptions::get_instance();
        $this->eltdMetaBoxes = AzaleaEltdfMetaBoxes::get_instance();
    }
    ...
    public function setSkin(AzaleaEltdfSkinAbstract $skinObject) {
        $this->skin = $skinObject;
    }
}

/**
 * Class AzaleaEltdfSkinManager
 *
...
/**
 * Class AzaleaEltdfSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class AzaleaEltdfSkinAbstract
{
    /**
     * @var string
     */
    ...
    public function loadTemplatePart($template, $params = array()) {
        ...
        if ($template !== '') {
            include(azalea_eltdf_find_template_path('framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php'));
        }
    }
    ...
    public function setShortcodeJSParams() { ?>
        <script>
            window.eltdSCIcon = '<?php echo azalea_eltdf_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
...
class AzaleaEltdfOptions
{
    ...
    public function getOptionValue($key) {
        ...
        if (array_key_exists($key, $azalea_eltdf_options)) {
            return $azalea_eltdf_options[$key];
...
/*
   Class: AzaleaEltdfAdminPage
   A class that initializes Elated Admin Page
*/

class AzaleaEltdfAdminPage implements iAzaleaEltdfLayoutNode
{
    ...
    function __construct($slug = "", $title = "", $icon = "") {
        $this->layout = array();
        $this->factory = new AzaleaEltdfFieldFactory();
        $this->slug = $slug;
        $this->title = $title;
        $this->icon = $icon;
    }
    ...
    public function renderChild(iAzaleaEltdfRender $child) {
        $child->render($this->factory);
    }
}

/*
   Class: AzaleaEltdfMetaBoxes
...
/*
   Class: AzaleaEltdfMetaBox
   A class that initializes Elated Meta Box
*/

class AzaleaEltdfMetaBox implements iAzaleaEltdfLayoutNode
{
    ...
    public function setTitle($label) {
        ...
        return $azalea_eltdf_Framework->getSkin()->getSkinLabel() . ' ' . $label;
    }
}

if (!function_exists('azalea_eltdf_init_framework_variable')) {
    function azalea_eltdf_init_framework_variable() {
        global $azalea_eltdf_Framework;

        $azalea_eltdf_Framework = AzaleaEltdfFramework::get_instance();
        $eltdSkinManager = new AzaleaEltdfSkinManager();
        $azalea_eltdf_Framework->setSkin($eltdSkinManager->getSkin());
    }

    add_action('azalea_eltdf_before_options_map', 'azalea_eltdf_init_framework_variable');
}
?>
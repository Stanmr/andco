...
/*
  Class: AzaleaEltdfIconCollections
...
class AzaleaEltdfIconCollections {
    ...
    private function initIconCollections() {
        $this->addIconCollection('font_awesome', new AzaleaEltdfIconsFontAwesome("Font Awesome", "fa_icon"));
        $this->addIconCollection('font_elegant', new AzaleaEltdfIconsFontElegant("Font Elegant", "fe_icon"));
	    $this->addIconCollection('ion_icons', new AzaleaEltdfIonIcons("Ion Icons", "ion_icon"));
        $this->addIconCollection('linea_icons', new AzaleaEltdfLineaIcons('Linea Icons', 'linea_icon'));
    }
    ...
    public function getSocialIconsMetaBoxOrOption($attributes) {
        ...
        if($type == 'meta-box') {
            azalea_eltdf_add_meta_box_field(
                array(
                    ...
                    'label' => $label,
                    'options' => array(
                        'font_awesome'      => esc_html__('Font Awesome', 'azaleawp'),
                        'font_elegant'      => esc_html__('Font Elegant', 'azaleawp'),
                        'ion_icons'         => esc_html__('Ion Icons', 'azaleawp')
                    ),
                    'args' => array(
                        'dependence' => true,
        ...
        foreach ($socialIconCollections as $collection_key => $collection_object) {
            ...
            $eltd_icon_pack_container = azalea_eltdf_add_admin_container(
                array(
                    'parent'			=> $parent,
    ...
    public function getVCParamsArray($iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false) {
        ...
        $iconPackParams = array(
            'type'        => 'dropdown',
            'heading'     => esc_html__('Icon Pack', 'azaleawp'),
        ...
        if(is_array($this->iconCollections) && count($this->iconCollections)) {
            foreach($this->iconCollections as $key => $collection) {
                $iconSetParams[] = array(
                    'type'        => 'dropdown',
                    'heading'     => esc_html__('Icon', 'azaleawp'),
    ...
    public function getSocialIconWidgetParamsArray() {
        ...
        $iconPackParams[] = array(
            'type'    => 'dropdown',
            'title'   => esc_html__('Icon Pack', 'azaleawp'),
            'name'    => 'icon_pack',
            'options' => array(
                'font_awesome' => esc_html__('Font Awesome', 'azaleawp'),
                'font_elegant' => esc_html__('Font Elegant', 'azaleawp'),
    ...
    public function getIconCollectionIcons(iAzaleaEltdfIconCollection $collection) {
...
if (!function_exists('azalea_eltdf_activate_theme_icons')) {
    function azalea_eltdf_activate_theme_icons() {
        global $azalea_eltdf_IconCollections;
        $azalea_eltdf_IconCollections = AzaleaEltdfIconCollections::get_instance();
    }

    add_action('after_setup_theme', 'azalea_eltdf_activate_theme_icons');
}
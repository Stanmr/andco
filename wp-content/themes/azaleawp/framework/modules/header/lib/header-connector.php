<?php
namespace AzaleaEltdf\Modules\Header\Lib;
/**
...
class HeaderConnector {
    ...
    public function connect($config = array()) {
        do_action('azalea_eltdf_pre_header_connect');
        ...
        if(!empty($config['affect_content'])) {
            add_filter('azalea_eltdf_content_elem_style_attr', array($this, 'contentMarginFilter'));
        }

        if(!empty($config['affect_title'])) {
            add_filter('azalea_eltdf_title_content_padding', array($this, 'titlePaddingFilter'));
        }

        do_action('azalea_eltdf_after_header_connect');
    }
     ...
     *
     * Hooks to azalea_eltdf_title_content_padding filter
     *
     * @return int
     */

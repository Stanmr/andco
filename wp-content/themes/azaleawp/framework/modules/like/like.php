...
class AzaleaEltdfLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_azalea_eltdf_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_azalea_eltdf_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'azaleawp' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'azaleawp' );
		}
...
if ( ! function_exists( 'azalea_eltdf_create_like' ) ) {
	function azalea_eltdf_create_like() {
		AzaleaEltdfLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'azalea_eltdf_create_like' );
}
...
if(!function_exists('azalea_eltdf_get_title')) {
    /**
     *
     ...
     */
    function azalea_eltdf_get_title() {
        $id = azalea_eltdf_get_page_id();

        extract(azalea_eltdf_title_area_height());
        extract(azalea_eltdf_title_area_background());
	    
        //check if title area is visible on page first, and set that as default value for filter
        $show_title_area = azalea_eltdf_get_meta_field_intersect('show_title_area') == 'yes' ? true : false;
        $show_title_area = apply_filters('azalea_eltdf_show_title_area', $show_title_area);

        if($show_title_area) {

            //check for title type on page first, then in options
            $type = azalea_eltdf_get_meta_field_intersect('title_area_type');

            //check if breadcrumbs are enabled on page first, then in options
            $enable_breadcrumbs_meta = azalea_eltdf_get_meta_field_intersect('title_area_enable_breadcrumbs');
            $enable_breadcrumbs = $enable_breadcrumbs_meta == 'yes' ? true : false;

            //check title tag value
            $title_tag_meta = azalea_eltdf_get_meta_field_intersect('title_area_title_tag');
	        ...
	        $subtitle_padding_meta = get_post_meta($id, 'eltdf_subtitle_side_padding_meta', true);
	        $subtitle_styles[] = !empty($subtitle_padding_meta) ? 'padding: 0 '.esc_attr($subtitle_padding_meta).'%;' : '';

	        $module = 'title';
	        $module = apply_filters('azalea_eltdf_title_module', $module);

            $layout = '';
            $layout = apply_filters('azalea_eltdf_title_layout', $layout);
            ...
            $parameters = apply_filters('azalea_eltdf_title_area_height_params', $parameters);

            azalea_eltdf_get_module_template_part('templates/titles/title', $module, $layout, $parameters);
        }
    }
}

if(!function_exists('azalea_eltdf_get_title_text')) {
    /**
     * Function that returns current page title text. Defines azalea_eltdf_title_text filter
     ...
     */
    function azalea_eltdf_get_title_text() {
        ...
        if (is_tag()) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'azaleawp');
        }
        ...
        elseif (is_author()) {
            ...
            $title = esc_html__('Author:', 'azaleawp') . " " . get_the_author();
        }
        ...
        elseif (is_search()) {
            ...
            $title = esc_html__('Search results for: ', 'azaleawp') . get_search_query();
        }

        //is single post page
        elseif (is_singular('post')) {
	        //get title for single post page
	        $title = azalea_eltdf_options()->getOptionValue('blog_single_title_in_title_area') == 'yes' ? get_the_title($id) : esc_html__('Blog', 'azaleawp');
        }

        //is current page 404?
        elseif (is_404()) {
            //is 404 title text set in theme options?
	        $title_404 = azalea_eltdf_options()->getOptionValue('404_title');
	        $title = !empty($title_404) ? $title_404 : esc_html__('404 - Page not found', 'azaleawp');
        }

        //is WooCommerce installed and is shop or single product page?
        elseif(azalea_eltdf_is_woocommerce_installed() && (azalea_eltdf_is_woocommerce_shop() || is_singular('product'))) {
        ...
        elseif(azalea_eltdf_is_woocommerce_installed() && (is_product_category() || is_product_tag())) {
            global $wp_query;
        ...
        elseif (is_archive()) {
            $title = esc_html__('Archive','azaleawp');
        }
        ...
        $title = apply_filters('azalea_eltdf_title_text', $title);

        return $title;
    }
}

if(!function_exists('azalea_eltdf_title_text')) {
    /**
     * Function that echoes title text.
     *
     * @see azalea_eltdf_get_title_text()
     */
    function azalea_eltdf_title_text() {
        echo azalea_eltdf_get_title_text();
    }
}

if(!function_exists('azalea_eltdf_subtitle_text')) {
    /**
     * Function that echoes subtitle text.
     */
    function azalea_eltdf_subtitle_text() {
        $id = azalea_eltdf_get_page_id();
        $subtitle 	= '';
...
if(!function_exists('azalea_eltdf_custom_breadcrumbs')) {
    /**
     * Function that renders breadcrumbs
     */
    function azalea_eltdf_custom_breadcrumbs() {
        global $post;

        $output = "";
        $homeLink = esc_url(home_url('/'));
        $pageid = azalea_eltdf_get_page_id();
        $bread_style = "";
        ...
        $delimiter = '<span class="eltdf-delimiter" '.azalea_eltdf_get_inline_style($bread_style).'>&nbsp; / &nbsp;</span>'; // delimiter between crumbs
        $home = esc_html__('Home', 'azaleawp'); // text for the 'Home' link
        $showCurrent = 1; // 1 - show current post/page title in breadcrumbs, 0 - don't show
        $before = '<span class="eltdf-current" '.azalea_eltdf_get_inline_style($bread_style).'>'; // tag before the current crumb
        $after = '</span>'; // tag after the current crumb

        if (is_home() && !is_front_page()) {
            $output = '<div class="eltdf-breadcrumbs"><div class="eltdf-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter . ' <a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . $homeLink . '">'. get_the_title($pageid) .'</a></div></div>';
        ...
        elseif(is_front_page()) {
            if ($showOnHome == 1) $output = '<div class="eltdf-breadcrumbs"><div class="eltdf-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a></div></div>';
        }

        else {

            $output .= '<div class="eltdf-breadcrumbs"><div class="eltdf-breadcrumbs-inner" itemprop="breadcrumb"><a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . $homeLink . '">' . $home . '</a>' . $delimiter;

            if ( is_category() || (azalea_eltdf_is_woocommerce_installed() && azalea_eltdf_is_product_category())) {
                ...
                $output .= $before . esc_html__('Search results for ', 'azaleawp') . '"' . get_search_query() . '"' . $after;

            } elseif ( is_day() ) {
                $output .= '<a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . get_year_link(get_the_time('Y')) . '">' . get_the_time('Y') . '</a>' . $delimiter;
                $output .= '<a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . get_month_link(get_the_time('Y'),get_the_time('m')) . '">' . get_the_time('F') . '</a>' . $delimiter;
            ...
            } elseif ( azalea_eltdf_is_woocommerce_installed() && is_singular('product') ){
	            $product = wc_get_product(get_the_ID());
	            
                if(get_option('woocommerce_shop_page_id')){
                    $output .= '<a itemprop="url" '. azalea_eltdf_get_inline_style($bread_style) .' href="' . get_permalink(get_option('woocommerce_shop_page_id')) . '">' . get_the_title(get_option('woocommerce_shop_page_id')) . '</a>' . $delimiter;
                }
                ...
                if($post->post_parent) {
                    ...
                    $output .= '<a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . get_permalink($parent) . '">' . $parent->post_title . '</a>';
                ...
                if($post->post_parent) {
                    ...
                    foreach($parent_ids as $parent_id) {
                        $output .= '<a itemprop="url" '.azalea_eltdf_get_inline_style($bread_style).' href="' . get_the_permalink($parent_id) . '">' . get_the_title($parent_id) . '</a>' . $delimiter;
                    }
                }
                ...
                $output .= $before . esc_html__('Posts tagged ','azaleawp') .'"' . single_tag_title('', false) . '"' . $after;

            } elseif ( is_author() ) {
                $author_id = get_query_var('author');
                $output .= $before . esc_html__('Articles posted by ','azaleawp') . get_the_author_meta('display_name', $author_id) . $after;

            } elseif ( is_404() ) {
                $output .= $before . esc_html__('Error 404','azaleawp') . $after;

            } elseif(azalea_eltdf_is_woocommerce_installed() && azalea_eltdf_is_woocommerce_shop()){
                $shop_id = get_option('woocommerce_shop_page_id');
            ...
            if ( get_query_var('paged') ) {
                ...
                $output .= $before . " (" . esc_html__('Page', 'azaleawp') . ' ' . get_query_var('paged') . ")" . $after;
            }
...
if(!function_exists('azalea_eltdf_get_title_area_height_default_value')) {
    /**
     * Function that returns title height
     **/
    function azalea_eltdf_get_title_area_height_default_value() {
        $title_height = 160;

        return apply_filters('azalea_eltdf_title_area_height_default_value', $title_height);
    }
}

if(!function_exists('azalea_eltdf_get_title_area_height')) {
    /**
     * Function that returns title height
     **/
    function azalea_eltdf_get_title_area_height() {
		$title_height_meta = azalea_eltdf_get_meta_field_intersect('title_area_height');
	
	    $title_height = !empty($title_height_meta) ? $title_height_meta : azalea_eltdf_get_title_area_height_default_value();

        return apply_filters('azalea_eltdf_title_area_height', $title_height);
    }
}

if(!function_exists('azalea_eltdf_get_title_content_padding')) {
    /**
     * Function that returns title content pading
     **/
    function azalea_eltdf_get_title_content_padding() {
        $title_padding = apply_filters('azalea_eltdf_title_content_padding', 0);
        
        return intval($title_padding); 
    }
}

if(!function_exists('azalea_eltdf_title_area_height')) {
    /**
     * Function that returns title height and padding to be applied in template
     **/

    function azalea_eltdf_title_area_height() {
        $title_height_and_padding = array();
        $title_height          = azalea_eltdf_get_title_area_height();
        $header_height_padding = azalea_eltdf_get_title_content_padding();
        $title_holder_height = '';
        $title_subtitle_holder_padding = '';

        //is responsive image is set for current page?
	    $is_img_responsive = azalea_eltdf_get_meta_field_intersect('title_area_background_image_responsive');
	
	    //check title area vertical alignment
	    $title_vertical_alignment = azalea_eltdf_get_meta_field_intersect('title_area_vertical_alignment');
...
if(!function_exists('azalea_eltdf_title_area_background')) {
    /**
     ...
     * @return array
     **/
    function azalea_eltdf_title_area_background() {
        $id = azalea_eltdf_get_page_id();
        $show_title_img = true;
	    ...
	    $background_color = azalea_eltdf_get_meta_field_intersect('title_area_background_color');

        //check if background image is set on page or in options
        $background_image = azalea_eltdf_get_meta_field_intersect('title_area_background_image');

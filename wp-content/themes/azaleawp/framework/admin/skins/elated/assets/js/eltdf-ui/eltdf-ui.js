			...
			frame: function() {
				...
				this._frame.on('update', function() {
					...
					jQuery.ajax({
						type: "post",
						url: ajaxurl,
						data: "action=azalea_eltdf_gallery_upload_get_images&ids=" + ids,
    ...
    */
	function eltdfInitAjaxForm() {
		...
		$(document).delegate(".eltdf_ajax_form", "submit", function (a) {
			var b = $(this);
			var c = {
				action: "azalea_eltdf_save_options"
			};
			jQuery.ajax({
				url: ajaxurl,
				cache: !1,
			...
			eltdfImportBtn.on('click', function(e) {
				...
				if (confirm(confirmMessage)) {
					...
					if(import_opt == 'content'){
						for( var i=1; i < 10; i++ ){
							var str;
							if (i < 10) str = 'azalea_content_0'+i+'.xml';
							else str = 'azalea_content_'+i+'.xml';
							jQuery.ajax({
								type: 'POST',
								url: ajaxurl,
								data: {

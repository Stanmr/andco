...
if(!function_exists('eltdf_core_portfolio_meta_box_functions')) {
	...
	add_filter('azalea_eltdf_meta_box_post_types_save', 'eltdf_core_portfolio_meta_box_functions');
	add_filter('azalea_eltdf_meta_box_post_types_remove', 'eltdf_core_portfolio_meta_box_functions');
}
...
if(!function_exists('eltdf_core_portfolio_scope_meta_box_functions')) {
	...
	add_filter('azalea_eltdf_set_scope_for_meta_boxes', 'eltdf_core_portfolio_scope_meta_box_functions');
}

if(!function_exists('eltdf_core_register_portfolio_cpt')) {
	function eltdf_core_register_portfolio_cpt($cpt_class_name) {
		$cpt_class = array(
			'EltdCore\CPT\Portfolio\PortfolioRegister'
		);
...
if(!function_exists('eltdf_core_get_archive_portfolio_list')) {
	function eltdf_core_get_archive_portfolio_list($eltdf_taxonomy_slug = '', $eltdf_taxonomy_name = '') {
		...
		$number_of_items = 12;
		$number_of_items_option = azalea_eltdf_options()->getOptionValue('portfolio_archive_number_of_items');
		if(!empty($number_of_items_option)) {
			$number_of_items = $number_of_items_option;
		}
		
		$number_of_columns = 4;
		$number_of_columns_option = azalea_eltdf_options()->getOptionValue('portfolio_archive_number_of_columns');
		if(!empty($number_of_columns_option)) {
			$number_of_columns = $number_of_columns_option;
		}
		
		$space_between_items = 'normal';
		$space_between_items_option = azalea_eltdf_options()->getOptionValue('portfolio_archive_space_between_items');
		if(!empty($space_between_items_option)) {
			$space_between_items = $space_between_items_option;
		}
		
		$image_size = 'landscape';
		$image_size_option = azalea_eltdf_options()->getOptionValue('portfolio_archive_image_size');
		if(!empty($image_size_option)) {
			$image_size = $image_size_option;
		}
		
		$item_layout = 'standard-shader';
		$item_layout_option = azalea_eltdf_options()->getOptionValue('portfolio_archive_item_layout');
		...
		$html = azalea_eltdf_execute_shortcode('eltdf_portfolio_list', $params);
...
if(!function_exists('eltdf_core_set_portfolio_single_info_follow_body_class')) {
	...
	function eltdf_core_set_portfolio_single_info_follow_body_class($classes) {
		if(is_singular('portfolio-item') && azalea_eltdf_options()->getOptionValue('portfolio_single_sticky_sidebar') == 'yes'){
...
if(!function_exists('eltdf_core_single_portfolio_title_display')) {
    ...
    function eltdf_core_single_portfolio_title_display($show_title_area) {
        if(is_singular('portfolio-item')) {
            ...
            $show_title_area_meta = azalea_eltdf_get_meta_field_intersect('show_title_area_portfolio_single');
    ...
    add_filter('azalea_eltdf_show_title_area', 'eltdf_core_single_portfolio_title_display');
}

if(!function_exists('eltdf_core_get_single_portfolio')) {
	function eltdf_core_get_single_portfolio() {
		$portfolio_template = azalea_eltdf_get_meta_field_intersect('portfolio_single_template');
...
if( !function_exists('eltdf_core_set_single_portfolio_style') ) {
	...
	function eltdf_core_set_single_portfolio_style($style) {
		...
		$class_prefix = azalea_eltdf_get_unique_page_class($id);
		...
		if(!empty($info_padding_top)) {
			$current_style['padding-top'] = azalea_eltdf_filter_px($info_padding_top).'px';
			
			$current_styles .= azalea_eltdf_dynamic_css($current_selector, $current_style);
		}
	...
	add_filter('azalea_eltdf_add_page_custom_style', 'eltdf_core_set_single_portfolio_style');
}
...
if(!function_exists('eltdf_core_get_portfolio_single_media_html')) {
	function eltdf_core_get_portfolio_single_media_html($media) {
		...
		if($media['type'] == 'image') {
			$params['lightbox'] = azalea_eltdf_options()->getOptionValue('portfolio_single_lightbox_images') == 'yes';
		...
		if(in_array($media['type'], array('youtube', 'vimeo'))) {
			$params['lightbox'] = azalea_eltdf_options()->getOptionValue('portfolio_single_lightbox_videos') == 'yes';
...
if(!function_exists('eltdf_core_portfolio_ajax_load_more')) {
	function eltdf_core_portfolio_ajax_load_more() {
		...
		$port_list = new \EltdCore\CPT\Shortcodes\Portfolio\PortfolioList();
		...
		if($query_results->have_posts()):
			while ( $query_results->have_posts() ) : $query_results->the_post();
				...
				$html .= eltdf_core_get_cpt_shortcode_module_template_part('portfolio', 'portfolio-item', $shortcode_params['item_type'], $shortcode_params);
			endwhile;
		else:

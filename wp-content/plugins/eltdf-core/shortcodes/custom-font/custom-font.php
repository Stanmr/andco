<?php
namespace EltdCore\CPT\Shortcodes\CustomFont;
...
class CustomFont implements Lib\ShortcodeInterface {
				...
				array(
					...
					'params'                    => array(
						...
						array(
							...
							'value'       => array_flip( azalea_eltdf_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'save_always' => true
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'value'       => array_flip( azalea_eltdf_get_font_weight_array( true ) ),
							'save_always' => true
						),
						array(
							'type'        => 'dropdown',
							...
							'value'       => array_flip( azalea_eltdf_get_font_style_array( true ) ),
							'save_always' => true
						),
						array(
							'type'       => 'textfield',
						...
						array(
							...
							'value'       => array_flip( azalea_eltdf_get_text_decorations( true ) ),
							'save_always' => true
						),
						array(
							'type'       => 'colorpicker',
	...
	private function getHolderStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . azalea_eltdf_filter_px( $params['font_size'] ) . 'px';
		}
		
		if ( ! empty( $params['line_height'] ) ) {
			$styles[] = 'line-height: ' . azalea_eltdf_filter_px( $params['line_height'] ) . 'px';
		}
		...
		if ( ! empty( $params['letter_spacing'] ) ) {
			$styles[] = 'letter-spacing: ' . azalea_eltdf_filter_px( $params['letter_spacing'] ) . 'px';
		}
	...
	private function getHolderData( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$data[] = 'data-font-size=' . azalea_eltdf_filter_px( $params['font_size'] );
		}
		
		if ( ! empty( $params['line_height'] ) ) {
			$data[] = 'data-line-height=' . azalea_eltdf_filter_px( $params['line_height'] );
		}

		if ( ! empty( $params['letter_spacing'] ) ) {
			$data[] = 'data-letter-spacing=' . azalea_eltdf_filter_px( $params['letter_spacing'] );
		}

...
if(!function_exists('azalea_eltdf_get_button_html')) {
    /**
     ...
     * @return mixed|string
     */
    function azalea_eltdf_get_button_html($params) {
        $button_html = azalea_eltdf_execute_shortcode('eltdf_button', $params);
...
if(!function_exists('eltdf_core_add_button_shortcodes')) {
	function eltdf_core_add_button_shortcodes($shortcodes_class_name) {
		$shortcodes = array(
			'EltdCore\CPT\Shortcodes\Button\Button'
		);

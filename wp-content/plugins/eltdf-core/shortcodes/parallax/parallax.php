<?php
namespace EltdCore\CPT\Shortcodes\Parallax;
...
class Parallax implements Lib\ShortcodeInterface {
		...
		if(function_exists('vc_map')) {
			vc_map(
				array(
					...
					'as_parent'                 => array( 'except' => 'vc_row, vc_accordion, eltdf_elements_holder_item' ),
					...
					'params'                    => array(
                        ...
                        array(
                            'type'       => 'dropdown',
                            'param_name' => 'parallax_full_height',
                            'heading'    => esc_html__( 'Set Parallax Full Height', 'eltdf-core' ),
						...
						array(
                            ...
                            'dependency'  => array( 'element' => 'parallax_full_height', 'value' => array( 'no' ) )
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'parallax_content_alignment',
							'heading'    => esc_html__( 'Set Parallax Content Vertical Alignment In Middle', 'eltdf-core' ),
							'value'      => array_flip( azalea_eltdf_get_yes_no_select_array( false, false ) )
						),
						array(
							'type'       => 'dropdown',
	...
	private function getHolderClass($params) {
		...
		if ($params['parallax_full_height'] === 'yes') {
			$itemClass[] = 'eltdf-parallax-full-height';
		}
	...
	private function getHolderStyles($params) {
		...
		if(!empty($params['parallax_height']) && ($params['parallax_full_height'] == 'no')) {
			$styles[] = 'min-height: '.azalea_eltdf_filter_px($params['parallax_height']).'px';
			$styles[] = 'height: '.azalea_eltdf_filter_px($params['parallax_height']).'px';
		} else {
            $styles[] = 'min-height: '.azalea_eltdf_filter_px($params['parallax_height']).'px';
            $styles[] = 'height: 100vh';
        }

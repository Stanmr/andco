...
if(!function_exists('eltdf_core_add_social_share_shortcodes')) {
	function eltdf_core_add_social_share_shortcodes($shortcodes_class_name) {
		$shortcodes = array(
			'EltdCore\CPT\Shortcodes\SocialShare\SocialShare'
		);
...
if(!function_exists('azalea_eltdf_get_social_share_html')) {
	/**
	 ...
	 * @return mixed|string
	 */
	function azalea_eltdf_get_social_share_html($params = array()) {
        if(azalea_eltdf_core_plugin_installed())
		    return azalea_eltdf_execute_shortcode('eltdf_social_share', $params);
	}
}

if (!function_exists('azalea_eltdf_the_excerpt_max_charlength')) {
	/**
	 * Function that sets character length for social share shortcode
	 * @param $charlength string original text
	 * @return string shortened text
	 */
	function azalea_eltdf_the_excerpt_max_charlength($charlength) {

		if (azalea_eltdf_options()->getOptionValue('twitter_via')) {
			$via = ' via ' . esc_attr(azalea_eltdf_options()->getOptionValue('twitter_via'));
		} else {
			$via = '';
		}

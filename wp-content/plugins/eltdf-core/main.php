...
add_action('after_setup_theme', array(EltdCore\CPT\PostTypesRegister::getInstance(), 'register'));

if(!function_exists('eltd_core_activation')) {
    /**
     * Triggers when plugin is activated. It calls flush_rewrite_rules
     * and defines azalea_eltdf_core_on_activate action
     */
    function eltd_core_activation() {
        do_action('azalea_eltdf_core_on_activate');

        EltdCore\CPT\PostTypesRegister::getInstance()->register();
        flush_rewrite_rules();
    }
...
if(!function_exists('eltd_core_theme_menu')) {
    ...
    function eltd_core_theme_menu() {
        if (eltdf_core_theme_installed()) {
            ...
            azalea_eltdf_init_theme_options();
            ...
            $page_hook_suffix = add_menu_page(
                ...
                'azalea_eltdf_theme_menu',                // The ID used to bind submenu items to this menu
                array($azalea_eltdf_Framework->getSkin(), 'renderOptions'), // The callback function used to render this menu
                $azalea_eltdf_Framework->getSkin()->getMenuIcon('options'),             // Icon For menu Item
                $azalea_eltdf_Framework->getSkin()->getMenuItemPosition('options')            // Position
            );

            foreach ($azalea_eltdf_Framework->eltdOptions->adminPages as $key=>$value ) {
                $slug = "";
                ...
                $subpage_hook_suffix = add_submenu_page(
                    'azalea_eltdf_theme_menu',
	                'Elated Options - '.$value->title,                   // The value used to populate the browser's title bar when the menu page is active
                    $value->title,                   // The text of the menu in the administrator's sidebar
                    'administrator',                  // What roles are able to access the menu
                    'azalea_eltdf_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
                    array($azalea_eltdf_Framework->getSkin(), 'renderOptions')
                );

                add_action('admin_print_scripts-'.$subpage_hook_suffix, 'azalea_eltdf_enqueue_admin_scripts');
                add_action('admin_print_styles-'.$subpage_hook_suffix, 'azalea_eltdf_enqueue_admin_styles');
            };

            add_action('admin_print_scripts-'.$page_hook_suffix, 'azalea_eltdf_enqueue_admin_scripts');
            add_action('admin_print_styles-'.$page_hook_suffix, 'azalea_eltdf_enqueue_admin_styles');
        }
    }
	...
	function eltd_core_theme_menu_backup_options() {
		if (eltdf_core_theme_installed()) {
			...
			$page_hook_suffix = add_submenu_page(
				...
				'administrator',                  // What roles are able to access the menu
				'azalea_eltdf_theme_menu'.$slug,                // The ID used to bind submenu items to this menu
				array($azalea_eltdf_Framework->getSkin(), 'renderBackupOptions')
			);
